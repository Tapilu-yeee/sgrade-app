...
- Mỗi trường "reason" và "evidence" tối đa 100 từ
            ...
            with st.spinner("🤖 AI đang phân tích 12 yếu tố theo phương pháp PwC..."):
                try:
                    ...
                    def fix_truncated_json(s):
                        """Fix JSON bị cắt giữa chừng bằng cách đếm ngoặc và tự đóng."""
                        s = s.strip().lstrip('\ufeff')
                        # Tìm vị trí bắt đầu {
                        start = s.find('{')
                        if start == -1:
                        ...
                        s = s[start:]
                        # Đếm ngoặc để biết cần đóng bao nhiêu
                        depth = 0
                        in_str = False
                        escape = False
                        last_valid = 0
                        ...
                        # JSON bị cắt — cần repair
                        # Cắt tại entry cuối hoàn chỉnh
                        # Tìm "}" cuối cùng trước khi JSON bị cắt
                        truncated = s
                        # Loại bỏ entry bị cắt dở: tìm },{ hoặc }] cuối
                        fixed = _re.sub(r',\s*\{[^}]*$', '', truncated)
                        fixed = _re.sub(r',\s*"[^"]*":\s*"[^"]*$', '', fixed)
                        # Đóng tất cả mảng và object còn mở
                        o_count = fixed.count('{') - fixed.count('}')
                        a_count = fixed.count('[') - fixed.count(']')
                        # Chuẩn chỉnh chuỗi
                        if not fixed.rstrip().endswith(','):
                            pass
                        else:
                            fixed = fixed.rstrip().rstrip(',')
                        fixed += ']' * max(0, a_count) + '}' * max(0, o_count)
                        return fixed

                    _fixed = fix_truncated_json(raw)
