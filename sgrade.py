        ...
        if not jd_content:
            ...
            with st.spinner("🤖 AI đang phân tích 12 yếu tố theo phương pháp PwC..."):
                try:
                    ...
                    import re as _re
                    _clean = raw.strip().lstrip('\ufeff')  # strip BOM
                    _clean = _re.sub(r'^[^{]*', '', _clean)  # strip anything before {
                    _clean = _re.sub(r'[^}]*$', '', _clean)  # strip anything after }
                    result = json.loads(_clean)
                    # Debug: show actual keys from first factor
                    if result.get("factors"):
                        _first = result["factors"][0]
                        st.caption(f"🔍 Debug keys: {list(_first.keys())}")
